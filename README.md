...
- **Model Architecture:** We initialized a 6-layer student model by extracting every alternate layer (0, 2, 4, 6, 8, 10) from the 12-layer `bert-base-uncased` teacher model.
...
3. **Data Preparation & Student Initialization:**
   ...
   ```
4. **Pre-training (Requires ~20 GPU Hours):**
   ```bash
   python pretrain_student.py   # Runs the full distillation training loop
   ```
   *(For the ablation study without distillation loss, run `python pretrain_student.py --ablation`)*
...
- **Parameter Reduction:** We verified a ~40% reduction in parameters (from ~110M to ~66M).
- **Inference Speed:** Our benchmarks confirmed that the 6-layer student model executes forward passes ~60% faster than the 12-layer teacher.
