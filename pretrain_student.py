...
# 1. Custom Trainer for Triple Loss
class DistillationTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        student_hidden = student_outputs.hidden_states[-1] # Final layer hidden states
        ...
        # B. Distillation Loss (Soft Cross-Entropy)
        loss_ce = F.kl_div(
        ...
        # C. Cosine Embedding Loss
        # Flatten the hidden states to align dimensions for cosine similarity
...
def setup_and_train(is_ablation=False):
    ...
    # Using 1% of the 10% subset for a rapid sanity check before the 20-hour run
    # Remove the .select() in production
    ...
    # Re-build student from Phase 1, but with the LM head attached
    ...
    # Transfer encoder weights and LM head weights
    ...
    # Data collator handles the dynamic 15% token masking
